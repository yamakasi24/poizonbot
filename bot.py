...
def callback_massage(callback):
    if callback.data == 'calculate':
        ...
        bot.send_message(callback.message.chat.id, f"Текущий курс юаня: {yuan_rate} ₽ \n \n"
                                                   ...
                                                   f"Курс Центробанка — это справочная цифра, по которой валюту никто не продает. В обменниках и банках курс будет всегда выше, так как туда включают наценки и комиссии за переводы 🇨🇳")
