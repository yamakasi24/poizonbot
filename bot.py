...
def go_to_main_menu(message):
    ...
    markup.add(btn9)
    btn10 = types.InlineKeyboardButton('Выкуп с других площадок 🛒', callback_data='otherMarket' )
    markup.add(btn10)
...
def callback_massage(callback):
    ...
    elif callback.data == 'otherMarket':
        
    bot.send_message(
        callback.message.chat.id,
        text="""❕Помимо Poizon, мы также можем выкупить и доставить товары с других китайских маркетплейсов: Taobao, Goofish (Xianyu), 95 и др.

Особенности площадок:

🔸Taobao — Минимальные цены за счёт прямых поставщиков. Огромный ассортимент включает как оригинальные товары, так и копии, поэтому важно анализировать продавцов и отзывы.

🐠Goofish — ресейл-площадка, много б/у товаров по сути как наш Авито. Можно найти редкие и лимитированные позиции или оригинальные вещи в хорошем состоянии по ценам ниже рынка. На площадке также есть возможность сразу проверить вещь на подлинность за дополнительную плату (~ 40¥)

🪀 95 — ресейл-площадка с полной проверкой: товар сначала поступает на склад, где его проверяют, фотографируют и точно оценивают состояние. За счёт этого получаешь реальные фото, честную оценку износа и минимальный риск фейка.

💲Расчет стоимости и условия доставки при выкупе с других площадок:

Наша комиссия за работу составляет 10% от стоимости заказа. По желанию можно оформить страховку — она составляет 3% от стоимости товара. Доставка оплачивается по факту веса:

Обычная доставка: 19-25 дней — 900₽/кг  
Экспресс доставка: 9-13 дней — 1190₽/кг  

Пример расчета стоимости заказа:
Вы заказали 5 позиций на сумму 1000¥

1000 * 1.10 * 12 = 13200₽ — итоговая цена без учета доставки, где 10% наша комиссия, а 12 — курс юаня к рублю.
"""
    )
