...
def go_to_main_menu(message):
    ...
    markup.add(btn9)
    btn10 = types.InlineKeyboardButton('Выкуп с других площадок 🛒', callback_data='market')
    markup.add(btn10)
...
def callback_massage(callback):
    ...
    elif callback.data == 'market':
        bot.send_message(callback.message.chat.id, f"❕Помимо Poizon, мы также можем выкупить и доставить товары с других китайских маркетплейсов: Taobao, Goofish (Xianyu), 95 и др."
                                                   f"\n\nОсобенности площадок:\n\n"
                                                   f"🔸Taobao — Минимальные цены за счёт прямых поставщиков. Огромный ассортимент включает как оригинальные товары, так и копии, поэтому важно анализировать продавцов и отзывы.\n\n"
                                                   f"🐠Goofish — ресейл-площадка, много б/у товаров по сути как наш Авито. Можно найти редкие и лимитированные позиции или оригинальные вещи в хорошем состоянии по ценам ниже рынка. На площадке также есть возможность сразу проверить вещь на подлинность за дополнительную плату (~40¥).\n\n"
                                                   f"🪀 95 — ресейл-площадка с полной проверкой: товар проходит складскую проверку, фотофиксацию и точную оценку состояния. Это даёт реальные фото и минимальный риск фейка.\n\n"
                                                   f"💲Расчет стоимости и условия доставки:\n\n"
                                                   f"Комиссия сервиса — 10% от стоимости заказа.\n"
                                                   f"Страховка (по желанию) — 3% от стоимости товара.\n\n"
                                                   f"Доставка по факту веса:\n\n"
                                                   f"Обычная: 19–25 дней — 900₽/кг\n"
                                                   f"Экспресс: 9–13 дней — 1190₽/кг\n\n"
                                                   f"Пример расчета стоимости заказа:\n"
                                                   f"Вы заказали 5 товаров на 1000¥\n\n"
                                                   f"1000 × 1.10 × 12 = 13200₽ - Это итоговая цена без учета доставки\n"
                                                   f"(где 10% комиссия, 12 — курс юаня)")
